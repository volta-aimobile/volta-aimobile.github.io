...
window.VoltaFeatures = (function () {
  ...
  // v42 (user, picture 1): "flip the arrow buttons in arabic so it looks
  // correct" — the dashboard plan pager mirrors for RTL: in Arabic
  // "previous" points RIGHT and "next" points LEFT (reading order flows
  // right → left). English keeps the classic left/right chevrons.
  ...
  // v17 (user: "make workout and diet plan in dashboard be synced with daily
  // discipline"): when the Daily Discipline tracker is following a plan, the
  // dashboard workout card renders the tracker's CURRENT day (label + list +
  // per-exercise done state) instead of the weekday-matched schedule page.
  // dashPlanMode — which source the last render used (tracker|schedule);
  // switching sources resets the pager to auto-follow. dashTrackerLastDay —
  // last seen tracker currentDay; when the discipline day advances, the
  // dashboard jumps back to following it.
  var dashPlanMode = '';
  var dashTrackerLastDay = -1;
  // v10 (user): "make the workout plan display 10 workouts at a time, and add
  // a '+x more' (x is the amount of workouts left) button" — the dashboard
  // workout card shows the first 10 exercise chips of the day; the rest sit
  // behind a "+X more" expander (X = remaining count). Collapses again when
  // the user flips to another day.
  ...
  function shiftDashboardPlanPage(delta) {
    ...
    dashExExpanded = false;   // v10: new day → collapsed list again
    try { renderDashboardPlan(); } catch (e) {}
  }
  function dashPlanPrevPage() { shiftDashboardPlanPage(-1); }
  function dashPlanNextPage() { shiftDashboardPlanPage(1); }

  // ── v29 (user, picture 2): ONE water source for the WHOLE app ──────────
  // "it says 1.3L/day, but it says 2.5L at the top, make sure that the water
  // number is always linked to the number at the top" — the dashboard's
  // DAILY WATER TARGET stat (top) computes LIVE from the profile weight
  // (weight × 0.033L), while the diet card read the STALE value stored in
  // the generated plan. Every water surface now calls THIS function so the
  // numbers can never disagree (top stat, dashboard diet card, Diet System
  // pills, the plan generator, and the AI coach snapshot via the stored
  // plan that renderHome keeps in sync).
  function waterTargetLiters() {
    try {
      var u = U();
      var w = (u && u.profile && Number(u.profile.weight)) || 0;
      if (!w || w < 25 || w > 400) {
        // no sane body weight on file → keep whatever the plan already says
  ...
  function renderDashboardPlan() {
    ...
    // v16 (user: "i get dinner only in diet plan"): upgrade legacy
    // dinner-only meal schedules in place before the dashboard renders.
    ...
    // ── v17: tracker state (synced with Daily Discipline) ──
    // deState is hydrated lazily: on a fresh boot the user can land on Home
    // before ever opening the Daily tab, so the persisted tracker plan
    // (u.dailyPlan) is loaded into deState here when needed.
    ...
    if (sched.length || trackerDays) {
      ...
      // v28 (user): "put a dumbbell next to workout plan" — the icon is BACK
      // on the card header (it was removed in v27, the user wants it again).
      ...
      // v17: pick the source — tracker (Daily Discipline) first, weekday
      // schedule as the no-tracker fallback.
      ...
      if (trackerDays && dashTrackerLastDay !== trackerCur) {
        dashTrackerLastDay = trackerCur;    // discipline day advanced → follow it
        dashPlanPage = -1;
      }
      if (trackerDays) {
        // ── TRACKER MODE: the same day the user is training right now ──
        ...
        html += '<div class="plan-day-head">' +
          // tracker labels are already localized + carry the focus (the SAME
          // label the Daily Discipline tab shows) — no dayLabel() remapping.
        ...
        if (exercises.length) {
          ...
          html += exercises.slice(0, shownCount).map(function (ex, ix) {
            ...
            // v28: no dumbbell icon on pending chips — done ones keep the check.
            // v29 (user): "remove rest from workout plan in dashboard" — the
            // REST chips are gone from the TRACKER-mode card too (same as the
            // schedule mode below); the Daily Discipline tab keeps the real
            // 60s rest timer.
      ...
      pageDays.forEach(function (day, i) {
        ...
        '</div>';
        if (exercises.length) {
          // v10: first 10 workouts visible; the rest behind "+X more".
          const shownCount = dashExExpanded ? exercises.length : Math.min(DASH_EX_VISIBLE, exercises.length);
          const hiddenCount = exercises.length - shownCount;
          const exMarkup = function (ex, ix) {
            const reps = ex.sets ? (ex.sets + '×' + (ex.reps || '10')) : (ex.reps || '');
            // v28: no dumbbell icon on the chips (user, picture 2).
            // v29 (user): "remove rest from workout plan in dashboard" — the
            // REST chips that v28 added after every 2nd exercise are GONE from
            // the dashboard card (the Daily Discipline tracker keeps its
            // functional 60s rest timer between exercise pairs).
      ...
      // v17: needs the generated schedule to exist (the popup renders it).
      if (sched.length) {
    ...
    if (d.dailyCalories || (d.mealSchedule && d.mealSchedule.length)) {
      ...
      html += '<div class="plan-split-card">' +
        '<div class="plan-split-head ph-diet">' +
          ...
          '</div>' +
        '</div>';
      // === v27 (user, picture 2): "make diet plan look exactly like workout
      // plan in dashboard" — the MyFitnessPal budget strip (big centered
      // "KCAL LEFT TODAY" number + progress bar) is GONE (the workout card has
      // no equivalent), and every meal now renders with the SAME markup and
      // classes as a workout-plan day row: .plan-day-row box, .plan-day-head
      // header (meal name + muted kcal on the right like "5 exercises") and
      // .plan-ex-grid with .plan-ex-chip chips for the food items. The chips
      // stay clickable (plan-food-chip-btn → jumpToGeneratedMeal to log). ===
      (d.mealSchedule || []).forEach(function (meal, i) {
        html += '<div class="plan-day-row" style="animation-delay:' + (i * 0.05) + 's">';
        html += '<div class="plan-day-head">' +
          '<span class="plan-day-name"><i class="fa-solid fa-bowl-food" style="color:var(--accent);font-size:.78rem;"></i> ' + esc(mealLabel(meal.type)) + '</span>' +
        ...
        if (sug && sug.length) {
          ...
          sug.forEach(function (s) {
            // v3 (user): "make meal buttons clickable and make the user be
            // able to log them" — each chip now jumps to the AI-Generated
            // Meals panel (or the meal info popup with the Log button).
            const name = (typeof s === 'string' ? s : (s.name || ''));
            const safe = String(name).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/"/g, '&quot;');
            html += '<button type="button" class="plan-ex-chip plan-food-chip-btn" onclick="jumpToGeneratedMeal(\'' + safe + '\')" aria-label="' + esc(name) + ' — open & log">' +
              esc(name) + '</button>';
          });
          html += '</div>';
        }
      ...
      if (d.hydrationLiters || typeof waterTargetLiters === 'function') {
        // v29 (user, picture 2): "make sure that the water number is always
        // linked to the number at the top" — the card no longer reads the
        // STALE plan value (1.3L); it computes LIVE from the same profile
        // weight the dashboard's DAILY WATER TARGET stat uses (2.5L), so the
        // two numbers can never disagree again.
  ...
  // v30 (user, picture 2): "put all daily exercises in 1 block that has all
  // workouts, instead of them taking space, and when the history gets
  // bigger, make a see more button" — each DAY now renders as ONE compact
  // block (all of that day's workouts inside it as tight mini-rows), only
  // the latest 3 day-blocks show by default and a "See more" button expands
  // the rest ("See less" collapses again).
  // v33 (user, picture 2): "make all daily exercise drills in 1 block and
  // when the user clicks on it, it displays all drills" — within each day,
  // sessions of the SAME sport (the repeated "Daily Exercise" drills!) are
  // now collapsed into ONE expandable row ("Daily Exercise · 12 drills" with
  // summed time/kcal). Tapping it expands the individual drill rows; tapping
  // again collapses them. Expansion is kept across re-renders in memory.
  var HISTORY_DAY_LIMIT = 3;
  var drillOpen = {};      // groupKey (date|sport) → expanded?
  var drillKeySeq = [];    // render-order keys → safe numeric onclick args
  ...
  function toggleBuilderExercise(name, muscle) {
    ...
    else builderState.picked.push({ name: name, muscle: muscle || '', sets: 2, reps: '10-12' });   // v27: 2 sets per exercise
    renderBuilderLibrary();
    renderBuilderSelected();
  }
...
(function () {
  ...
  var globals = [
    ...
    'openHistoryPopup', 'closeHistoryPopup'   // v42 (user: "the x button doesnt work") — the Workout
                                              // History popup's X calls closeHistoryPopup() INLINE from
                                              // index.html; without this global the click threw a
