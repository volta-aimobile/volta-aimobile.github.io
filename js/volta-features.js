...
window.VoltaFeatures = (function () {
        ...
        if (sug && sug.length) {
          ...
          sug.forEach(function (s) {
            ...
            html += '<button type="button" class="plan-ex-chip plan-food-chip-btn" onclick="jumpToGeneratedMeal(\'' + safe + '\')" aria-label="' + esc(name) + ' — open & log">' +
              '<i class="fa-solid fa-check"></i>' + esc(name) + '</button>';
          });
          html += '</div>';
        }
        html += '</div>';
  ...
  function renderHistory() {
    ...
    // Day-level summary numbers (whole minutes / whole kcal)
    ...
    const shown = historyExpanded ? dates : dates.slice(0, HISTORY_DAY_LIMIT);
    let html = '';
    shown.forEach(function (date, gi) {
      let label = date;
      try {
      ...
      groups[date].forEach(function (s, i) {
        ...
        const noteLine = s.note ? '<small class="history-note"><i class="fa-solid fa-note-sticky"></i> ' + esc(s.note) + '</small>' : '';
        html += '<div class="history-mini-row" style="animation-delay:' + (Math.min(i, 8) * 0.04) + 's;">' +
          '<span class="history-ico"><i class="fa-solid ' + sportIcon(s.sport) + '"></i></span>' +
          '<div class="history-main"><b>' + esc(sportName || s.sport) + '</b>' + noteLine + '</div>' +
    ...
    // "See more" / "See less" toggle when there are hidden day-blocks
    if (dates.length > HISTORY_DAY_LIMIT) {
      const hidden = dates.length - shown.length;
      html += '<button type="button" class="history-see-more" onclick="VoltaFeatures.toggleHistory()">' +
        (historyExpanded
          ? '<i class="fa-solid fa-chevron-up"></i> ' + t('See less', 'عرض أقل')
          : '<i class="fa-solid fa-chevron-down"></i> ' + t('See more', 'عرض المزيد') + (hidden > 0 ? ' (' + hidden + ')' : '')) +
        '</button>';
    }
    el.innerHTML = html;
  }
  ...
  API.toggleHistory = toggleHistory;
