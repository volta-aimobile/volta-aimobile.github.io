...
const VoltaDB = (function () {
  ...
  // ─── Internal: HALAL sanitizer (user request — remove bacon / pork /
  // ham / alcohol / sushi meals from the local IndexedDB) ─────────────────
  // Devices that seeded the meal store BEFORE the halal cleanup still hold
  // the old entries (seeding only runs on an EMPTY store). This pass removes
  // every stored meal whose name or ingredients match a haram pattern, so
  // existing users see the same clean database as fresh installs.
  ...
  function init() {
    ...
    initPromise = (async function () {
      try {
        db = await open();
        await seedIfEmpty();
        // Halal pass (runs on every boot — cheap cursor scan, idempotent:
        // nothing to delete after the first clean run).
        await sanitizeMeals();
  ...
  const meals = {
    ...
    async filter(opts) {
      ...
      if (opts.diet) {
        ...
        // v12: gluten-free no longer rubber-stamps omnivore meals — meals
    ...
    /** Halal compliance check used by the app shell (exposed for tests). */
    isHaram: isHaramMeal
  };
