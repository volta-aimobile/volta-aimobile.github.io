...
window.VoltaAI = (function () {
  ...
  function recoveryScreenHtml() {
    return head('fa-battery-three-quarters', 'RecoveryIQ', 'ريكفري آيكيو',
  ...
  function renderRecoveryLocked() {
    ...
    holder.innerHTML =
      '<div class="vai-recov-locked">' +
        ...
        '<h3>' + T('RecoveryIQ is a Premium feature', 'ريكفري آيكيو ميزة بريميوم') + '</h3>' +
  ...
  function ensureRecoveryScreen() {
    const holder = document.getElementById('recovery-screen-container');
    if (!holder) return;
    // v29 (user): RecoveryIQ is one of the 4 on-device AI features — unlocked
    // for everyone (aiFree), so the screen never renders its locked panel.
    const premium = (typeof VoltaPremium === 'undefined') ||
                   !!(VoltaPremium.aiFree && VoltaPremium.aiFree('recovery')) ||
  ...
  function recoveryHtml(data) {
    ...
    if (data.workout && data.workout.blocks && data.workout.blocks.length) {
      html += '<div class="vai-answers-head"><i class="fa-solid fa-person-walking"></i> ' + T2('YOUR RECOVERY WORKOUT', 'تمرين الاستشفاء الخاص بك') + ' · ' + data.workout.totalMin + ' ' + T2('MIN', 'دقيقة') + '</div>' +
        '<div class="vai-recov-session">' +
          ...
          '<button class="vp-cta vp-cta-blue vai-recov-log-btn" onclick="VoltaAI.logRecoverySession()"><i class="fa-solid fa-check-to-slot"></i> ' + T2('Log this recovery session', 'سجّل جلسة الاستشفاء') + '</button>' +
        '</div>';
    }
  ...
  async function getRecovery() {
    ...
    if (!body) return;
    body.innerHTML = '<div style="padding:18px;color:var(--muted);"><i class="fa-solid fa-spinner fa-spin"></i> ' + T('RecoveryIQ is reading your week…', 'ريكفري آيكيو يقرأ أسبوعك…') + '</div>';
  ...
  // the generated recovery session as a REAL 'Stretching' session (MET 2.3,
  // Light) with the workout's own duration, so it lands in the rings, the
  // streaks history and the weekly charts like any other session.
  ...
  function logRecoverySession() {
    ...
    try {
      ...
      const cals = Math.max(1, Math.round(2.3 * 3.5 * weight / 200 * w.totalMin));
      user.sessions.push({
        sport: 'Stretching',
