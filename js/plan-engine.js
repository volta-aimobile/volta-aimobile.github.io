...
window.VoltaPlan = (function () {
  ...
  // plan-engine mirrors volta.js's equipment helpers. They live on window
  // by the time generation runs (user finished the survey), with local
  // fallbacks in case of odd load orders.
  ...
  function pickExercises(muscleGroups, opts) {
    ...
    var perGroup = opts.perGroup || 0;      // v28: N per group when > 0
    var goal = opts.goal || null;
    var rng = opts.seed != null ? makeRng(opts.seed) : Math.random;
    // v12: robust no-equipment detection ("None", "no", "nothing", ""…).
    var noEquip = __peUserNoEquip(equipment);
    // v28 (user): perGroup mode — the day target = groups × N.
    ...
    if (typeof WORKOUTS_DB !== 'undefined' && WORKOUTS_DB.length) {
      WORKOUTS_DB.forEach(function (w) {
        ...
        // Equipment filter: if user has no equipment, only Bodyweight
        // workouts — and no bar/rig exercises (v12: "Bodyweight" pull-ups,
        // chin-ups, hanging raises, inverted rows need a bar).
        if (noEquip) {
        ...
        seen[w.name] = true;
        // v27 (user): "2 sets per exercise" — every level now gets 2 sets
        // (the old level-based 3/4 sets are gone). Rep ranges stay.
        var sets = 2;
    ...
    if (pool.length < perExercise && typeof WORKOUT_DB !== 'undefined') {
      muscleGroups.forEach(function (mg) {
        (WORKOUT_DB[mg] || []).forEach(function (name) {
          if (seen[name]) return;
          // v12: no equipment → skip equipment-requiring hardcoded entries
    ...
    if (perGroup > 0) {
      // v28: take EXACTLY N per muscle group (reality caps at pool size) —
      // the shuffled pool is scanned once and each group fills its quota.
      var quota = {};
  ...
  function buildDietPlan(profile, survey, goalPlan) {
    ...
    // Hydration: 0.033L per kg body weight.
    // v29 (user, picture 2): "make sure that the water number is always linked
    // to the number at the top" — the survey-answer multiplier is GONE: the
    // dashboard's top DAILY WATER TARGET stat computes weight × 0.033 live,
    // so the generated plan now uses the EXACT same formula. Every water
    // surface in the app (top stat, diet card, Diet System pill, AI coach)
    // shows one number, always.
    ...
    var distribution;
    if (mealsPerDay === 1) {
      // v16 (user): "i get dinner only in diet plan" — answering "1 meal" in
      // the survey used to collapse the whole day into a single Dinner row,
      // which read as a broken diet plan. The diet plan now ALWAYS covers
      // the full day (breakfast/lunch/dinner); the meals-per-day answer still
      // feeds the AI coach context, but the plan structure stays standard.
      distribution = [
  ...
  function generate(user) {
    ...
    // v3 FIX (user): "make training days be according to the survey, not 3
    // for everyone" — '3-4 days' used to map to 3, so nearly every user got
    // a 3-day plan. It now maps to 4 (upper end of their chosen range), and
    // unknown/missing answers fall back to 3.
    var daysPerWeek = 3;
    ...
    // v27 (user): '60+ minutes' was REMOVED from the survey — legacy answers
    // that picked it now generate like 45-minute sessions.
    ...
    // v28 (user): NEW FULL-BODY FORMULA — every training day trains ALL SIX
    // groups (Chest, Back, Legs, Shoulders, Arms, Core) with N exercises per
    // group from the survey session time:
    //   15 minutes → 2 per group (12 exercises),
    //   30 minutes → 4 per group (24 exercises),
    //   45 minutes → 5 per group (30 exercises),
    // every exercise 2 sets, and the workout list shows 1 rest after every
    // 2 exercises. (Replaces the v27 0.3/min total-count ratio.)
    ...
    var workoutSchedule = rawSchedule.map(function (day) {
      ...
      // v28: interleave by group so consecutive exercises don't repeat the
      // same muscle (the pick pool order is shuffled; regroup + round-robin).
      var byGroup = {};
  ...
  function dietFilterFor(dietPref) {
    ...
    // v12 (user): "make sure gluten free and all those people get the proper
    // food" — Gluten-free mapped now (it previously returned null = NO
    // filtering at all in the diet-plan suggestions).
  ...
  // v12: gluten-free no longer rubber-stamps every omnivore meal — meals
  // tagged "Allergen: Gluten" (or with obvious gluten ingredients) are
  // excluded; naturally gluten-free food stays.
  ...
  function renderSummary(plan, lang) {
    ...
    plan.workout.schedule.forEach(function (day) {
      ...
      // v31: compact focus chip (volta.js owns the helper; inline fallback
      // for safety when loaded standalone).
      var cfn = (typeof compactPlanFocus === 'function') ? compactPlanFocus : function (f) {
        var s = (f || '').trim(); if (!s) return '';
        var p = s.split(',').map(function (x) { return x.trim(); }).filter(Boolean);
        return (p.length <= 2) ? p.join(', ') : (p[0] + ' +' + (p.length - 1));
      };
      html += '<div class="plan-day-header"><b>' + day.day + '</b> <span class="plan-day-focus" title="' + (day.focus || '') + '">' + cfn(day.focus) + '</span></div>';
