...
window.VoltaNotifications = (function () {
  ...
  // ─── v43: native reminder bridge (Android APK) ────────────────────────
  // The APK shell exposes window.VoltaNative (Java). When the PWA runs in a
  // browser there is no such object and every call is skipped. The native
  // side fires the daily workout reminder via AlarmManager even when the
  // app is CLOSED — the exact behavior the PWA gets from the service
  // worker's scheduled notifications.
  function syncNativeReminders(on) {
    try {
      var nat = window.VoltaNative;
      if (!nat) return;                                  // plain browser → no-op
      // v43: keep the native reminder in the APP language (ar → Arabic text)
  ...
  function refreshSettingsUI() {
    try {
      ...
      if (line) {
        ...
        // v43 (user): "remove the option to select when to get notifications
        // in settings" — the time is no longer shown OR selectable.
  ...
  function dailyCheckTick() {
    try {
      ...
      // v43: stamp the trained day for the NATIVE alarm (APK) — when the
      // user already logged minutes today the AlarmManager reminder skips,
      // exactly like this in-page engine does.
      try {
