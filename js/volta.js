...
// ===== v44: sliding segmented-toggle thumb =====
// "in settings … put an animation for the slider when it moves and make it
// slides". One pill (.toggle-thumb) glides under the active .toggle-btn of
// every .toggle-group; JS measures the active button (offsetLeft/Width) so
// any group size, label width and both LTR/RTL directions work.
function positionToggleThumbs() {
  try {
    document.querySelectorAll('.toggle-group').forEach(function (group) {
      ...
      if (!active || !group.offsetWidth || !active.offsetWidth) return; // hidden tab
      const x = active.offsetLeft;
      const w = active.offsetWidth;
      const first = !thumb.style.transform && !thumb.style.width;
      if (first) thumb.style.transition = 'none';   // snap silently on first paint
...
// ===== v44: Arabic-database text-node walker =====
// "make a whole database for arabic and replace all the english texts with
// the ones in the database so nothing lags at all". After every render,
// every text node whose trimmed content is a key of the static Arabic
// database (js/lang/ar-db.js) or the runtime sport/workout name maps is
// swapped to its Arabic entry — plain hash lookups: zero network, zero AI,
// zero lag. Switching back to English restores the original text via the
// data-tx-en marker attribute left on the parent element.
function arDbTranslateWalk() {
  const db = window.VOLTA_AR_DB;
  if (!db) return;
  const isAr = (store.lang === 'ar');
  // Merge the runtime name maps (declared later in this file) exactly once.
  try {
  ...
  roots.forEach(function (root) {
    ...
    nodes.forEach(function (node) {
      ...
      if (!key) return;
      if (isAr) {
        // already swapped with this exact key? nothing to do
        ...
        // English mode — restore DB-swapped nodes to their original text.
...
function applyTranslations() {
  try {
    ...
    // 5) v44: Arabic-database walker — swap every DB-known English text
    //    node to Arabic instantly (and restore on the way back to English).
    try { arDbTranslateWalk(); } catch (e) {}
    // 6) v44: keep the sliding toggle thumb under the active segment
    //    (label widths changed above when the language flipped).
...
async function openGooglePicker() {
  ...
  // v36: inside the Android APK (file:// asset build) Firebase popups are
  // blocked by the OS, so the local account flow is used directly.
  if (!isCoach && !window.VOLTA_NATIVE && window.VoltaGoogle && window.VoltaGoogle.isConfigured()) {
    try {
      await window.VoltaGoogle.signInAndEnter();
      return;
    } catch (e) { /* fall through to the local flow */ }
  }
...
function showTab(name) {
  ...
  // v30: snappy whoosh on every tab switch (respects the Sound Effects
  // setting — VoltaSounds.play checks it internally).
  ...
  // v44: groups in a just-shown tab were display:none — slide thumbs now.
...
function renderChart() {
  ...
  // Round 15 → v41: the tab-entrance cascade (vSlideUpTab) animates panels
  // for ~380ms — a mid-animation measure locks the canvas at a shrunken
  // size (the graph "never quite fills its box" glitch). Re-measure once
  // the entrance animation has settled — but ONLY when the panel is
  // actually visible (renders also happen while the Home tab is
  // display:none).
  setTimeout(function () {
    try {
...
function renderSports() {
  ...
  // v44 (user, picture 1): the stat cards (Top sport / totals) kept the
  // OLD language's values when the Sports tab rendered — the tracker
  // renderer writes them, so run it here too.
...
function suggestionsForSlot(type, kcalTarget, count) {
  try {
    ...
    const push = function (arr) {
      (arr || []).forEach(function (m) {
        if (m && m.name && !seen[m.name]) {
          seen[m.name] = 1;
          // v41: chips follow the app language too (same name guard as the
          // AI meal cards — Latin nameEn surfaces in English UI).
    ...
    try {
      // v43: chips follow the SAME language layer as the cards — the day's
      // set is translated (not regenerated) on a language switch.
...
// upgrades those in place.
// ===== v46 (user): "also bring back snacks" =====
// (a) fewer than 2 meal rows → rebuilt to the standard 4-row split
//     (breakfast/lunch/dinner/snack, existing dailyCalories/macros re-split
//     25/30/30/15, hydration untouched, suggestions left for the normal
//     fillers). Idempotent — after one heal the schedule has 4 rows.
// (b) a schedule WITHOUT a snack row gets one appended: the existing rows'
//     kcal/macro targets are rescaled ×0.85 and the snack takes 15% — so
//     already-saved 3-row plans gain their Snacks row on the next render,
//     no regeneration needed. Idempotent (a snack row short-circuits it).
function healSingleMealDietPlan(u) {
  try {
    ...
    // (a) legacy dinner-only / single-row schedules → full 4-row day
    ...
    // (b) v46: any saved schedule without a snack row gains one in place
...
function showMealInfo(name) {
  ...
  // v42 (user: "meal more info button doesnt load"): last resort — the exact
  // card set currently on screen. A rendered card must never dead-end.
...
function showGeminiMealInfo(idx) {
  try {
    // v43: the card set is language-translated on render — the info modal
    // must show the SAME translated names the card shows.
    let rec = geminiApplyLang(geminiMealsToday());
    let m = rec ? (rec.meals || [])[idx] : null;
    // v42 (user: "meal more info button doesnt load"): if the day-cache
    // record went stale between render and tap (language / diet / day
    // switch), fall back to the EXACT card set on screen — a visible card
    // must always open its info modal.
...
// All images are LOCAL files so they work offline and never 404 like the
// old CDN links.
...
// v47 (user): the img/ folder was SPLIT into img1..img4 — alphabetical
// buckets by FILE name (a–c → img1, d–h → img2, i–r → img3, s–z → img4).
// Each bucket keeps the exercises/ + exercises/se/ sub-structure inside it,
// and the three dashboard banners sit directly in img1/. voltaImgBucket()
// is the single source of truth for the mapping — it must stay in sync
// with scripts/split-img-v47.py, which did the physical move on disk.
...
// {imgN}/exercises/se/{bucket}.jpg. This maps a RESOLVED classic photo path
// to its guide counterpart ('img2/exercises/pushup.jpg' →
// 'img2/exercises/se/pushup.jpg'). v47: the pre-split legacy path
// ('img/exercises/pushup.jpg') is still recognised and re-pointed at the
// right bucket, so nothing breaks while old strings drain out of caches.
// Non-local paths (CDN leftovers) map to ''.
...
function renderDailyExercise() {
  ...
  if (deState.step === 1) {
    ...
    // v33 (user, picture 1): "remove the text at the top and keep day 1 of 30,
    // but make it bigger" — the long plan-label headline ("Tue · Chest, Back,
    // Legs, … (Week 1)") that used to sit above the day counter is REMOVED.
    // "Day N of M" is now the ONLY title of the day card, promoted to the
    // hero heading (blue day number kept via .de-day-num).
    const dayNumHtml = ar
...
function wdAssistHtml(w) {
  ...
  // v43 (user): premium features are premium AGAIN — the chips render the
  // PREMIUM lock badge for free users and open the paywall when tapped.
...
function toggleWorkoutAssist(kind) {
  ...
  // v43 (user): AI assist features are premium-gated again — free users go
  // to the paywall hub when they tap a chip.
...
function wdAssistStartWithWorkout() {
  ...
  // v43 (user): premium-only again — free users' workouts just run without
  // the auto-started AI engine (the Settings → AI Camera OFF switch also
  // silences it for premium users).
...
function voltaMealPrompt(lang) {
  // v41: native-language scanner prompts — same principle as the meal
  // generator: the prompt itself is written in the app language so the
  // result comes back natively in that language (name included).
  if (lang === 'ar') {
...
async function analyzeMealWithGeminiDirect(base64Image, lang) {
  ...
  const key = (typeof window.VOLTA_GEMINI_KEY === 'string' && window.VOLTA_GEMINI_KEY) || 'AQ.Ab8RN6LS0BA1_UyKlIM_dksXh9DiP0AJrIiMeR9_RNVCaFly-Q';
  const body = {
...
// v41 (user): "make the prompt get sent with the current app language" —
// the prompt ITSELF is now written in the app's language (a full native
// Arabic prompt for Arabic users, English otherwise) so Gemini thinks and
// answers natively in that language: no post-generation translation pass,
// no mixed-language output, faster and better-quality results.
// v41 (user): "every ai plan is in arabic … make the meal name get
// translated too" — the ENGLISH prompt now carries a HARD rule that the
// meal NAME must always be Latin/English letters (transliterated local
// dish names like "Ful Medames", never Arabic script), and the Arabic
// prompt demands Arabic names — so the name matches the app language
// exactly like the description already did.
...
function voltaNormalizeGeminiMeals(arr, lang, dietPref) {
  ...
  (Array.isArray(arr) ? arr : []).forEach(function (m) {
    ...
    out.push({
      ...
      // v44: bilingual generation — the Arabic (or English) twin fields ride
      // in the SAME response, so a language switch never waits on the API.
  ...
  for (let i = 0; i < VOLTA_MEAL_GEMINI_MODELS.length; i++) {
    ...
    try {
      ...
      const record = {
        ...
        lang: lang,       // v43: the BASE generation language. Switching the app
                          // language no longer regenerates — the same set is
                          // translated via geminiTranslateMeals and cached in
                          // rec.translations.
        translations: {}, // v43: per-language translation maps { ar: {idx:{name,desc,ingredients[]}} }
    ...
    if (r && r.day === new Date().toDateString() && (r.meals || []).length >= 4) {
      if (!r.email || !store.session || r.email === store.session) {
        if (String(r.diet || 'None') === String(currentDietPref())) {
          // v43 (user): "dont change meals when the user changes languages,
          // just translate the existing generated meals to the current
          // language" — the day's set is NO LONGER language-scoped. A set
          // generated in the other language stays THE SAME SET; the render
          // path (geminiApplyLang + geminiTranslateMeals) shows it in the
          // current language by translating it, not by regenerating it.
          // (Only a DIET change still regenerates — that's a real food
          // preference change.)
          return r;
        }
      }
...
// ─── v43: language layer for the day's Gemini meals ───────────────────
// The cache record keeps the meals in the language they were GENERATED in
// (rec.lang) plus any translations the user already fetched:
//   rec.translations = { ar: { <mealIndex>: {name, desc, ingredients[]} }, en: {...} }
// geminiApplyLang() returns a NON-mutating copy of the record with the
// meals expressed in the CURRENT app language (translated when available).
...
function geminiApplyLang(rec) {
  ...
  const meals = (rec.meals || []).map(function (m, i) {
    ...
    // v44 ARABIC DATABASE FIRST — zero-lag paths:
    //   1. inline bilingual fields (nameAr/descAr/ingredientsAr arrived in
    //      the SAME generation response — one API call, nothing to wait for)
    //   2. the static Arabic database (js/lang/ar-db.js, instant hash read)
    //   3. the v43 cached translations (rec.translations[cur])
    //   4. last resort: the runtime translation call (legacy caches only)
    if (cur === 'ar') {
      ...
      // English UI: records generated in Arabic carry nameEn (+descEn).
      ...
      // v44: static-DB REVERSE path — an Arabic-named meal with no nameEn
      // (legacy caches) is restored to English instantly from the DB's
      // reverse index. No network, no lag.
  ...
  // A language pass is "covered" when no meal needed the slow path.
...
// ─── v41: meal-name script guards ──────────────────────────────────────
// Belt-and-suspenders on top of the native-language prompts: if an older
// cached meal (or a stubborn model answer) still carries a name in the
// wrong script, the card shows the Latin "nameEn" instead — the meal NAME
// now follows the app language exactly like the description always did.
...
function geminiDisplayName(m) {
  if (!m) return '';
  if (store.lang === 'ar') {
    // v44: Arabic straight from the bilingual response or the static DB —
    // both instant; no translation request is involved.
    if (m.nameAr && hasArabicScript(m.nameAr)) return m.nameAr;
    const db = window.VOLTA_AR_DB;
    if (db) {
  ...
  if (hasArabicScript(m.name) && isLatinScript(m.nameEn)) return m.nameEn;
  // v44: static-DB reverse path — Arabic name, legacy cache, instant EN.
  if (hasArabicScript(m.name)) {
    const db = window.VOLTA_AR_DB;
    const enFromDb = (db && db.rev) ? db.rev[String(m.name).trim()] : null;
    if (enFromDb) return enFromDb;
  }
  return m.name;
}
...
function renderDietMealsCards(meals, meta) {
  ...
  if (gemini) {
    // v42 (user): "you brought back the blue frame behind 'regenerate another
    // diet plan' button, remove it including location" — the LOCATION line
    // (pin + city/country) is GONE and the bar is FRAMELESS now: a small
    // muted caption + the clean regenerate action, no box around them.
    html += '<div class="gemini-meal-src">' +
      ...
      '</div>';
    // v43 (user): language-switch status lines — what the cards are doing
    // right now instead of a silent language mix.
  ...
  html += (meals || []).map(function (m, idx) {
    // v41: the meal NAME now follows the app language too — Arabic-script
    // names surface as their Latin nameEn when the app runs in English.
    const dispName = gemini ? geminiDisplayName(m) : getMealName(m);
    // v43: translation-safe "already logged" check — the log stores a
    // stable refKey (the meal's BASE name) so switching the app language
    // (and the displayed name with it) can never cause a double log.
    ...
    // v41 (user, carried from the "remove that icon next to the meal name"
    // request): the floating blue info button is GONE from the meal cards.
    // v43 (user: "more info button in meals work but its invisible, make it
    // visible"): the name-tap target STAYS, but every card now also carries
    // a clearly VISIBLE "More info" button (ghost chip under the Log
    // button) that opens the very same info modal — discoverable, labeled,
  ...
  // v42: remember the EXACT card set on screen. The info-modal and log
  // handlers fall back to this registry when the day-cache lookup misses
  // (stale record after a language/diet/day switch) — a card that is
  // visible must ALWAYS open its info and log correctly.
...
async function maybeShowGeminiMeals() {
  ...
  try {
    ...
    if (rec) {
      // v43: SAME meals, CURRENT language. If the set was generated in the
      // other language the cards render immediately (base language) with a
      // small "translating…" status, then geminiTranslateMeals swaps in the
      // translation the moment it arrives. Offline → explanatory note.
    ...
    } else if (!rec && !navigator.onLine) {
      // v43 (user): "do the same for features that need network when
      // offline" — an explanatory offline note instead of silence.
      const panel = document.getElementById('diet-meals');
      if (panel && !panel.querySelector('.gemini-meal-offline-note')) {
        const ar = (store.lang === 'ar');
        const note = document.createElement('div');
        note.className = 'gemini-meal-offline-note';
        note.innerHTML = '<i class="fa-solid fa-wifi"></i><span>' + (ar
          ? 'أنت غير متصل بالإنترنت — تعرض الآن وجباتك المحلية، وسيولّد الذكاء الاصطناعي وجباتك عند عودة الاتصال.'
          : 'You\'re offline — showing your local meal picks. AI-generated meals will load once you\'re back online.') + '</span>';
        panel.insertBefore(note, panel.firstChild);
      }
    }
...
// ─── v43: translate the day's EXISTING Gemini meals (never regenerate) ──
// User: "dont change meals when the user changes languages, just translate
// the existing generated meals to the current language, and when its
// loading, put a text explaining that its loading".
// One Gemini call translates the whole cached set (name + desc +
// ingredients) into the current app language; the result is persisted in
// rec.translations[cur] so every later switch to that language is instant
// and offline-safe. The meal SET itself never changes.
...
async function geminiTranslateMeals(rec) {
  ...
  try {
    ...
    if (list && list.length) {
      ...
      // Merge into the FRESH record — the user may have regenerated or
      // re-logged while the request was in flight.
      ...
      // Dropped offline mid-translation → swap the spinner for the
      // explanatory offline note (cards stay in their original language).
      ...
      // v43 fix: online but the translation call failed → clear the
      // "Translating your meals…" status so it never spins forever; the
      // cards keep their original language and can be re-translated on
...
function logGeminiMeal(idx) {
  try {
    // v43: same language layer as the cards — the rendered (translated)
    // name is what goes in the log, and the stable base-name refKey keeps
    // the once-per-day dedupe intact across language switches.
    let recBase = geminiMealsToday();
    let rec = recBase ? geminiApplyLang(recBase) : null;
    let m = rec ? (rec.meals || [])[idx] : null;
    // v42: same on-screen fallback as the info modal — the Log button of a
    // visible card must work even when the day-cache record went stale.
    if (!m && window.__voltaMealCards && window.__voltaMealCards.gemini) {
      m = window.__voltaMealCards.meals[idx] || null;
    }
    if (!m) return;
...
function voltaMealShowResult(data) {
  ...
  // v41: the scanned meal's NAME follows the app language like everything
  // else — an Arabic-script name is shown as its Latin nameEn when the app
  // runs in English (mutation is safe: this object is display-only from
  // here on — the panel text and the log entry both read .name).
  if (!ar && data && hasArabicScript(data.name) && isLatinScript(data.nameEn)) {
    data.name = data.nameEn;
  }
  lastAIMealResult = data;
  // v44 (user, picture 3 + Arabic DB): the result follows the app language
  // from the SAME response — nameAr/descAr are baked into the one API call
  // (zero translation lag) and unit labels localize inline.
  const shownName = (!ar && hasArabicScript(data.name) && isLatinScript(data.nameEn))
    ? data.nameEn
    : (ar && data.nameAr && hasArabicScript(data.nameAr) ? data.nameAr : data.name);
  const shownDesc = (ar && data.descAr) ? data.descAr : data.description;
  document.getElementById('ai-meal-name').textContent = shownName || 'Unknown meal';
  document.getElementById('ai-meal-desc').textContent = shownDesc || '';
  document.getElementById('ai-meal-kcal').textContent = (data.calories || 0) + ' ' + (ar ? 'سعرة' : 'kcal');
  document.getElementById('ai-meal-protein').textContent = (ar ? 'ب' : 'P') + ': ' + (data.protein || 0) + 'g';
  document.getElementById('ai-meal-carbs').textContent = (ar ? 'ك' : 'C') + ': ' + (data.carbs || 0) + 'g';
  document.getElementById('ai-meal-fat').textContent = (ar ? 'د' : 'F') + ': ' + (data.fat || 0) + 'g';
...
function registerServiceWorker() {
  // v36: copies that set window.VOLTA_NO_SW (mobile-test frame, APK assets)
  // never register the service worker — two copies sharing one SW cache
  // would fight over the cache version.
