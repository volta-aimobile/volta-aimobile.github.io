/**
 * Volta Coach AI — the in-app coach chat (v32: cloud-AI backed)
 * ══════════════════════════════════════════════════════════════════════
 ...
 *
 * v32 (user: "i dont like the current ai state, remove all of this"):
 * the offline-AI kit was REMOVED — open questions now stream REAL answers
 * from OpenRouter (js/volta-cloud-ai.js). Commands + quick data answers
 * stay instant & offline.
 * ══════════════════════════════════════════════════════════════════════ */
window.VoltaCoachChat = (function () {
  ...
  // v32: ml of water the coach has tracked for the CURRENT user today
  // (written by the cloud-AI command parser via the app bridge).
  ...
  var INTENTS = [
    {
      ...
      fn: function (s) {
        return T('Hey' + (s.name ? ' ' + s.name : '') + '! ⚡ I\'m your Volta coach — quick data answers are instant on-device, and open questions get real AI answers from the cloud. Ask me about your workout today, your calories, your plan, hydration — or anything fitness.',
                'أهلاً' + (s.name ? ' ' + s.name : '') + '! ⚡ أنا مدربك في فولتا — الإجابات السريعة عن بياناتك فورية على جهازك، والأسئلة المفتوحة تجيب عليها ذكاءات سحابية حقيقية. اسألني عن تمرين اليوم أو سعراتك أو خطتك أو أي شيء عن اللياقة.');
      }
    },
    {
    ...
    },
    {
      ...
      fn: function (s) {
        ...
        // v31: if the coach has tracked water today (via "log 500 ml water"),
        // surface the REAL number — the app UI only shows the daily target.
  ...
  // ─── intent matcher (extracted so the offline orchestrator can decide
  // whether the fast brain already owns this question) ─────────────────
  // Returns the answer text when an intent matched, or null when the
  // question should be handed to the heavier on-device AI pipeline.
  ...
  function brain(q) {
    ...
    return T('I\'m your Volta coach — I know your profile, plan, sessions and diet log, and the cloud AI brain handles everything else. Try asking:\n• ' + (s.todayWorkouts.length ? '“What\'s my workout today?”' : '“How many calories are left?”') + '\n• “Am I drinking enough water?”\n• “How\'s my progress?”\n• “Best football drills for stamina?” — any open fitness question goes to the cloud',
            'أنا مدربك في فولتا — أعرف ملفك وخطةتك وجلساتك وسجل غذائك، والذكاء السحابي يتولى الباقي. جرّب أن تسأل:\n• ' + (s.todayWorkouts.length ? '«ما تمارين اليوم؟»' : '«كم سعرة متبقية؟»') + '\n• «هل أشرب ماءً كافياً؟»\n• «كيف تقدمي؟»\n• «أفضل تدريبات للياقة كرة القدم؟» — أي سؤال مفتوح يذهب للذكاء السحابي');
  }
  ...
  // tier badges — shows WHICH layer answered (v32: instant local data,
  // in-app actions, or the OpenRouter cloud brain)
  var BADGES = {
  ...
  // badge for a cloud-AI result — which layer answered
  ...
  // one turn through the cloud AI (v32): logging/navigation commands
  // execute locally in the app instantly; open questions stream real
  // OpenRouter answers token-by-token into a live bubble.
  function cloudTurn(q) {
    var typing = appendTyping();
    ...
    window.VoltaCloudAI.handle(q, {
      ...
      finish(out && out.text ? out.text : brain(q), badgeFor(out));
    }).catch(function () {
      finish(brain(q), 'fast');
    });
  }
  ...
  function ask(q) {
    if (!q || !q.trim()) return;
    if (state.busy) return;             // one AI turn at a time
    q = q.trim();
    state.history.push({ who: 'me', q: q });
    renderHistory();

    // 0) COMMANDS FIRST — logging ("log 500 ml water", "i ate a 400 kcal
    //    sandwich") and navigation ("open diet") execute locally in the
    //    app: instant, offline, and never spend cloud quota.
  ...
  function continueAsk(q) {
    // 1) fast path — the app's own intents answer instantly from the
    //    user's own data (no cloud, no latency, no quota spent)
    var quick = tryIntents(q);
    if (quick != null) { replySoon(quick, 'fast'); return; }

    // 2) everything else → REAL cloud AI (OpenRouter, streamed)
    if (window.VoltaCloudAI) {
      cloudTurn(q);
      return;
    }

    // 3) cloud module absent → legacy local brain
